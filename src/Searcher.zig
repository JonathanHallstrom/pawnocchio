    ...
    while (mp.next()) |scored_move| {
        ...
        if (!is_root and !is_pv and best_score >= evaluation.matedIn(MAX_PLY)) {
            ...
            if (is_quiet) {
                ...
                const captured_value = if ((&board.mailbox)[move.to().toInt()].opt()) |cpt| SEE.value(cpt.toPieceType()) else 0;
                const bnfp_futility_value = static_eval + captured_value + 100 + depth * 100;
