...
fn search(
        ...
        !is_singular_search)
    {
        ...
        if (depth <= 3 and eval + tunable_constants.razoring_offs + tunable_constants.razoring_mult * depth <= alpha) {
    ...
    const easy_capture_mask = board.occupancyFor(stm.flipped()) & board.lesser_threats[stm.toInt()];
    ...
    while (mp.next()) |scored_move| {
        ...
        const score = blk: {
            ...
            if (depth >= 3 and num_searched > 1) {
                ...
                reduction -= @as(i32, 1024) * @intFromBool(move.to().toBitboard() & easy_capture_mask != 0);
