    ...
    while (mp.next()) |scored_move| {
        ...
        if (score > alpha) {
            ...
            if (score >= beta) {
                score_type = .lower;
                if (is_quiet) {
                    self.histories.updateQuiet(board, move, cur.prev, root.history.bonus(depth + @intFromBool(cutnode and depth < 4)));
                    for (searched_quiets.slice()) |searched_move| {
                        if (searched_move == move) break;
                        self.histories.updateQuiet(board, searched_move, cur.prev, -root.history.penalty(depth + @intFromBool(cutnode and depth < 4)));
                    }
                }
                break;
            }
