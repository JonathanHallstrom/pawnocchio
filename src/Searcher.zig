...
fn search(
        ...
        !is_singular_search)
    {
        ...
        if (eval >= beta +
            ...
            @divTrunc(cur.history_score, 400) * @intFromBool(!cur.move_is_noisy))
        {
    ...
    while (mp.next()) |scored_move| {
        ...
        const score = blk: {
            ...
            if (depth >= 3 and num_searched > 1) {
                ...
                reduction -= @as(i32, 512) * @intFromBool(tt_hit and tt_pv and tt_entry.flags.score_type.givesLowerBound() and tt_entry.score > alpha);
