...
//             for (0..64) |sq| {
//                 res[2 * p + 0][sq] = mg_value[p] + mg_pesto_table[p][sq];
//                 res[2 * p + 1][sq] = mg_value[p] + mg_pesto_table[p][sq ^ 56];
//             }
//         }
//         break :blk res;
//     };
...
fn pawnEval(
    ...
    res += @popCount(first.getOverlap(pawns).toInt()) * 1;

    const second = BitBoard.fromSquareUnchecked("A4").allRight()
        .getCombination(BitBoard.fromSquareUnchecked("A5"));
    res += @popCount(second.getOverlap(pawns).toInt()) * 2;

    const third = BitBoard.fromSquareUnchecked("A6").allRight()
        .getCombination(BitBoard.fromSquareUnchecked("A7"));
    res += @popCount(third.getOverlap(pawns).toInt()) * 4;
