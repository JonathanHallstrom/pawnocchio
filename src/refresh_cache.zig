    ...
    fn refresh(noalias self: *NNCacheEntry, comptime stm: Colour, board: *const Board, noalias acc: [*]i16, mirror: nnue.MirroringType) void {
        ...
        while (num_adds >= 4) : (num_adds -= 4) {
            for (0..nnue.L1_SIZE) |i| {
                self.accumulator[i] +=
                    (&nnue.weights.hidden_layer_weights)[adds[num_adds - 4] * nnue.L1_SIZE + i] +
                    (&nnue.weights.hidden_layer_weights)[adds[num_adds - 3] * nnue.L1_SIZE + i] +
                    (&nnue.weights.hidden_layer_weights)[adds[num_adds - 2] * nnue.L1_SIZE + i] +
                    (&nnue.weights.hidden_layer_weights)[adds[num_adds - 1] * nnue.L1_SIZE + i];
            }
        }
        ...
        while (num_subs >= 4) : (num_subs -= 4) {
            for (0..nnue.L1_SIZE) |i| {
                self.accumulator[i] -=
                    (&nnue.weights.hidden_layer_weights)[subs[num_subs - 4] * nnue.L1_SIZE + i] +
                    (&nnue.weights.hidden_layer_weights)[subs[num_subs - 3] * nnue.L1_SIZE + i] +
                    (&nnue.weights.hidden_layer_weights)[subs[num_subs - 2] * nnue.L1_SIZE + i] +
                    (&nnue.weights.hidden_layer_weights)[subs[num_subs - 1] * nnue.L1_SIZE + i];
            }
        }
