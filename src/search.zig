...
fn search(
        ...
        excluded.isNull())
    {
        ...
        if (depth <= 5 and tt_corrected_eval >= beta + tunable_constants.rfp_mult * (depth -| @intFromBool(improving))) {
            return result(beta + @divFloor(tt_corrected_eval - beta, 3), move_buf[0]);
        }
    ...
    for (move_buf[0..move_count], 0..) |move, i| {
        ...
        const is_losing = best_score <= eval.mateIn(MAX_SEARCH_DEPTH);
        const history_score = if (move.isQuiet()) move_ordering.getHistory(turn, board, move, previous_move) else 0;
        if (!pv and !is_losing) {
            if (prune_quiets and move.isQuiet() and !move.isPromotion())
                continue;

            if (!is_in_check and move.isQuiet() and depth <= 4 and history_score < tunable_constants.history_pruning_mult * depth) {
                prune_quiets = true;
                continue;
            }
            ...
            tt_entry.tp != .upper)
        {
            ...
            if (score < s_beta) {
                extension += 1;
                if (!pv and score < s_beta - tunable_constants.double_extension_margin)
                    extension += 1;
            } else if (tt_entry.score >= beta) {
                extension -= 1;
                if (!pv and tt_entry.score >= beta + tunable_constants.double_negext_margin) { // from potential
                    extension -= 1;
                }
            }
        }
