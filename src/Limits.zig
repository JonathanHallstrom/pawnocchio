...
fn computeEvalStabilityFactor(_: *const Limits, stab: i64) u64 {
    return @intCast(@max(900, tunable_constants.eval_stab_base - @divTrunc(tunable_constants.eval_stab_offs * stab, 1024)));
}

fn computeMoveStabilityFactor(_: *const Limits, stab: i64) u64 {
    return @intCast(@max(950, tunable_constants.move_stab_base - @divTrunc(tunable_constants.move_stab_offs * stab, 1024)));
}
