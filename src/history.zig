...
pub const HistoryTable = struct {
    ...
    ttmove_corrhist: [6 * 64][2]CorrhistEntry,
    ...
    pub fn reset(self: *HistoryTable) void {
        ...
        @memset(std.mem.asBytes(&self.ttmove_corrhist), 0);
    }
    ...
    pub fn updateCorrection(self: *HistoryTable, board: *const Board, prev: TypedMove, ttmove: TypedMove, corrected_static_eval: i32, score: i32, depth: i32) void {
        ...
        self.ttmove_corrhist[@as(usize, ttmove.tp.toInt()) * 64 + ttmove.move.to().toInt()][board.stm.toInt()].update(err, weight);
    }

    pub fn correct(self: *const HistoryTable, board: *const Board, prev: TypedMove, ttmove: TypedMove, static_eval: i16) i16 {
        ...
        const ttmove_correction: i32 = (&self.ttmove_corrhist)[@as(usize, ttmove.tp.toInt()) * 64 + ttmove.move.to().toInt()][board.stm.toInt()].val;

        const correction = (pawn_correction + nonpawn_correction + countermove_correction + ttmove_correction) >> 8;
