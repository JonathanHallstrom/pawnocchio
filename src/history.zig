...
pub const NoisyHistory = struct {
    ...
    inline fn entry(self: anytype, board: *const Board, move: TypedMove, good_see: bool) root.inheritConstness(@TypeOf(self), *i16) {
        const from_offs: usize = move.move.from().toInt();
        const to_offs: usize = move.move.to().toInt();
    ...
    pub inline fn updateRaw(self: *NoisyHistory, board: *const Board, move: TypedMove, good_see: bool, upd: i32) void {
        gravityUpdate(self.entry(board, move, good_see), upd);
    }

    inline fn update(self: *NoisyHistory, board: *const Board, move: TypedMove, good_see: bool, depth: i32, is_bonus: bool) void {
        self.updateRaw(board, move, good_see, if (is_bonus) bonus(depth) else -penalty(depth));
    }

    inline fn read(self: *const NoisyHistory, board: *const Board, move: TypedMove, good_see: bool) i16 {
        return self.entry(board, move, good_see).*;
    }
};
...
pub const HistoryTable = struct {
    ...
    pub fn readNoisy(self: *const HistoryTable, board: *const Board, move: Move, good_see: bool) i32 {
        const typed = TypedMove.fromBoard(board, move);
        var res: i32 = 0;
        res += self.noisy.read(board, typed, good_see);

        return res;
    }

    pub fn updateNoisy(self: *HistoryTable, board: *const Board, move: Move, good_see: bool, depth: i32, is_bonus: bool) void {
        const typed = TypedMove.fromBoard(board, move);
        self.noisy.update(board, typed, good_see, depth, is_bonus);
    }
