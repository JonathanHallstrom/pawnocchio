...
pub noinline fn scoreMove(board: *const Board, move: Move, threshold: i32, comptime mode: Mode) bool {
    ...
    if (board.isEnPassant(move)) {
        occ &= ~move.getEnPassantPawnSquare().toBitboard();
    }
...
test scoreMove {
    ...
    try std.testing.expect(!scoreMove(&(Board.parseFen("7k/8/8/3Pp3/8/8/7K/4r3 w - e6 0 1", false) catch unreachable), Move.enPassant(.d5, .e6), 1, .testing));
}
