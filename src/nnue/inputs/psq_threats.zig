...
pub fn Context(comptime B: type) type {
    return struct {
        ...
        fn needsThreatRefresh(self: *Self, parent_ply: u16, col: Colour, king_sq: Square) bool {
        ...
        fn refreshThreatHalf(self: *Self, ply: u16, col: Colour, board: *const B, weights: *const arch.Weights) void {
        ...
        inline fn collectPawnIdsVBMI2(board: *const B, col: Colour, sq_mask: u8, exclude: u64) @Vector(64, u8) {
        ...
        inline fn collectPawnIdsScalar(board: *const B, col: Colour, sq_mask: u8, exclude: u64) [16]u8 {
        ...
        fn collectPawnPairs(board: *const B, col: Colour, sq_mask: u8, exclude: u64, indices: []u16) usize {
            const friendly_bb = board.pawnsFor(col) & ~exclude;
            const enemy_bb = board.pawnsFor(col.flipped()) & ~exclude;
            var n: usize = 0;
        ...
        fn collectAllPawnPairs(out: []u16, board: *const B, col: Colour) usize {
        ...
        noinline fn buildThreatAccumulator(buf: *Accumulator, board: *const B, col: Colour, weights: *const arch.Weights) void {
            @branchHint(.cold);
        ...
        fn materialiseThreatHalf(self: *Self, ply: u16, col: Colour, board: *const B, weights: *const arch.Weights) void {
        ...
        fn ppSqMask(board: *const B, col: Colour) u8 {
        ...
        fn anyMaskedPair(board: *const B, dp: *const DirtyPawns) bool {
        ...
        fn computePairs(board: *const B, col: Colour, dp: *const DirtyPawns, add_out: []u16, sub_out: []u16) PPCounts {
            if (!arch.PAWN_PAIR_INPUTS) comptime unreachable;
            const sq_mask = ppSqMask(board, col);
        ...
        // reference impl
        fn computePairsSimple(board: *const B, col: Colour, dp: *const DirtyPawns, sq_mask: u8, r0_id: u8, r1_id: u8, a_id: u8, add_out: []u16, sub_out: []u16) PPCounts {
        ...
        inline fn computePairsVBMI2(board: *const B, col: Colour, dp: *const DirtyPawns, sq_mask: u8, r0_id: u8, r1_id: u8, a_id: u8, add_out: []u16, sub_out: []u16) PPCounts {
            const r0 = dp.removed[0];
            const r1 = dp.removed[1];
            const a = dp.added;
        ...
        fn threatStacks(self: *Self) nnue_threat_updates.Stacks {
...
pub fn Handle(comptime B: type) type {
    return struct {
        ...
        weights: *const arch.Weights,
        ply: u16,
        psq_frame: *psq.State(B),
        threat_frame: *ThreatState,

        pub fn addSub(self: Self, add: root.PSQTFeature, sub: root.PSQTFeature) void {
            std.debug.assert(self.psq_frame.dirty_piece.isClean());
            self.psq_frame.dirty_piece = .initMove(add, sub);
        }

        pub fn addSubSub(self: Self, add: root.PSQTFeature, sub1: root.PSQTFeature, sub2: root.PSQTFeature) void {
            std.debug.assert(self.psq_frame.dirty_piece.isClean());
            self.psq_frame.dirty_piece = .initCapture(add, sub1, sub2);
        }

        pub fn addAddSubSub(self: Self, add1: root.PSQTFeature, add2: root.PSQTFeature, sub1: root.PSQTFeature, sub2: root.PSQTFeature) void {
            std.debug.assert(self.psq_frame.dirty_piece.isClean());
            self.psq_frame.dirty_piece = .initCastle(add1, add2, sub1, sub2);
        }

        pub fn threatOnChange(self: Self, board: *const B, piece: ColouredPieceType, sq: Square, comptime is_add: bool) void {
            nnue_threat_updates.onChange(&self.threat_frame.threat_updates, self.ctx.threatStacks(), board, piece, sq, is_add);
        }

        pub fn threatOnMove(self: Self, board: *const B, old_piece: ColouredPieceType, src: Square, new_piece: ColouredPieceType, dst: Square) void {
            nnue_threat_updates.onMove(&self.threat_frame.threat_updates, self.ctx.threatStacks(), board, old_piece, src, new_piece, dst);
        }

        pub fn threatOnMutate(self: Self, board: *const B, old_piece: ColouredPieceType, new_piece: ColouredPieceType, sq: Square) void {
