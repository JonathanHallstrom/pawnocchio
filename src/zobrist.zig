...
var data: [bytes_per_side * 2]u8 = undefined;
...
noinline fn init() void {
    ...
    // add some extra entropy for good measure
...
pub fn get(piece: Piece, turn: lib.Side) u64 {
    if (!initialized) {
        initialized = true;
        init();
    }
    const offset = @intFromEnum(piece.getType()) * bytes_per_piece + piece.getLoc() + if (turn == .white) bytes_per_side else 0;
    return std.mem.readInt(u64, data[offset..][0..8], @import("builtin").cpu.arch.endian());
}
