...
const ResolvedTunables = std.enums.EnumArray(TuningSchemaField, tuning_schema.Tunable);

fn FactorizedFamilyValues(comptime spec: tuning_schema.FactorizedTunable, comptime Value: type) type {
    comptime var field_names: [spec.max_order][]const u8 = undefined;
    comptime var field_types: [spec.max_order]type = undefined;
    inline for (0..spec.max_order) |order| {
        field_names[order] = tuning_schema.factorizedOrderFieldName(order);
        field_types[order] = [tuning_schema.factorizedInteractionCount(spec, order)]Value;
    }
